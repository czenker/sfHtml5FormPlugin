...
$w = new sfWidgetFormInputDate();
$now = time();
$dateNow = date('Y-m-d', $now);

$t->is($w->render('foobar'), '<input type="date" name="foobar" id="foobar" />', 'render empty tag ok');
...
try {
	$t->is($w->formatDate($dateNow), $dateNow, 'formatDate accepts RFC3339-full-date and renders correctly');
}
catch(InvalidArgumentException $e) {
	$t->fail('formatDate accepts RFC3339-full-date');
}
...
try {
	$t->is($w->formatDate('2007-02-19 12:34:56'), '2007-02-19', 'formatDate accepts yyyy-mm-dd hh:ii:ss');
}
catch(InvalidArgumentException $e) {
...
try {
	$t->is($w->formatDate('2007-02-19'), '2007-02-19', 'formatDate accepts yyyy-mm-dd');
}
catch(InvalidArgumentException $e) {
